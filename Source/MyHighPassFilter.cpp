...
MyHighPassFilter::MyHighPassFilter(float cutoff, float Q, float sampleRate)
{
...
void MyHighPassFilter::setCutoff(float cutoff)
{
	mCutoffFrequency = cutoff;
	mComputeFilterCoefficients();
}

void MyHighPassFilter::setQ(float Q)
{
	mQ = Q;
	m1_Q = 1.0f / mQ;
	mComputeFilterCoefficients();
...
void MyHighPassFilter::setFilter(float cutoff, float Q)
{
	mCutoffFrequency = cutoff;
	mQ = Q;
	m1_Q = 1.0f / mQ;
...
void MyHighPassFilter::setSampleRate(float sampleRate)
{
	mSampleRate = sampleRate;
...
float MyHighPassFilter::filter(float input)
{
